(function() {
    window.app = window.app || { }//it will define the variable app to an empty object if it is not already defined.
    ...
    function Home(boards) {
        return `<header class="shadow header container fixed" >
                    <h3 class="no-margin">Project Planner</h3>
                </header>
                <main class="vertical-center">
                    <div class="border-radius shadow bg-white w-50 auto-margin ">
                        <div class="flex border-bottom padding">
                            ...
                            <button id="add-board" class="icon-button color-success">Add<i class="lnr lnr-plus-circle"></i></button>
                        </div>
                        ${boards.length !== 0 ? boards.reduce((acc,board)=>acc += BoardButton(board), '') : '<div class="padding color-gray text-center">No boards</div>'}
    ...
    function Board({id, title, lists}) {
        return `<header class="header shadow container">
                    <div class="flex">
    ...
    function List({id, title, items, total}) {
